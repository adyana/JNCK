...
contract Crowdsale is Ownable, ReentrancyGuard {
	...
	/* how much has been raised by crowdale (in JNCK) */
	uint256 public amountRaisedJNCK;
	...
	/* the balances (in JNCK) of all investors */
	mapping(address => uint256) public balanceOfJNCK;
	...
	function checkJNCKFunds(address addr) external view returns (uint256) {
		return balanceOfJNCK[addr];
	}
    ...
    *  this method allows to purchase tokens in behalf of another address.*/
    function invest() public onlyWhitelisted payable {
		...
		balanceOfJNCK[msg.sender] = balanceOfJNCK[msg.sender].add(amount.mul(price));
		amountRaisedJNCK = amountRaisedJNCK.add(amount.mul(price));
	...
	function getJNCK() external afterClosed nonReentrant {
		require(balanceOfJNCK[msg.sender] > 0, "non-contribution");
		uint256 amount = balanceOfJNCK[msg.sender];
		uint256 balance = tokenReward.balanceOf(address(this));
		require(balance >= amount, "lack of funds");
		balanceOfJNCK[msg.sender] = 0;
	...
	function withdrawJNCK() external onlyOwner {
		uint256 balance = tokenReward.balanceOf(address(this));
		require(balance > 0, "zero-JNCK-balance");
